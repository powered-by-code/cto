...
export const capturePosthogData = (): void => {
  try {
    ...
    setTimeout(() => {
      ...
      console.log("PostHog data stored in localStorage:", {
        sessionId,
        distinctId,
        deviceId,
      });
  ...
  try {
    ...
    if (userData.email) {
      posthog.people.set({
        ...
        ...(storageData.assessmentResults && {
          assessmentResults: storageData.assessmentResults,
        }),
        ...(storageData.quizAnswers && {
          quizAnswers: storageData.quizAnswers,
        }),
      });
    }
    ...
    posthog.capture(eventName, {
      ...
      ...(eventName === EventNames.IT_BUDGET_OPTIMIZER &&
        storageData.calculatorData && {
...
export const trackEvent = async (
  ...
  if (
    eventName === EventNames.IT_BUDGET_OPTIMIZER &&
    (eventData.action === "downloaded_pdf" ||
      eventData.action === "requested_report")
  ) {
    updateUserStorage({
      calculatorData: {
...
const trackPlunk = async (
  ...
  try {
    ...
    const posthogData = {
      posthogSessionId:
        storageData.posthogSessionId || posthog.get_session_id() || "",
      posthogDistinctId:
        storageData.posthogDistinctId ||
        posthog.get_property("distinct_id") ||
        "",
      posthogDeviceId:
        storageData.posthogDeviceId || posthog.get_property("$device_id") || "",
    };
    ...
    const data: Record<string, string | number | boolean> = {
      ...
      name: userData.name || storageData.name || "",
      firstName:
        userData.name?.split(" ")[0] || storageData.name?.split(" ")[0] || "",
      jobTitle: userData.jobTitle || storageData.jobTitle || "",
